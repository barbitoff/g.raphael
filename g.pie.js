    ...
    function Piechart(paper, cx, cy, r, values, opts) {
        ...
        if (len == 1) {
            ...
            for (i = 0; i < len; i++) {
                ...
                var p = paper.path(opts.init ? ipath : path).attr({ fill: opts.colors && opts.colors[(opts.preserveColorOrder?values[i].order:i)] || chartinst.colors[i] || "#666", stroke: opts.stroke || "#fff", "stroke-width": (opts.strokewidth == null ? 1 : opts.strokewidth), "stroke-linejoin": "round" });
